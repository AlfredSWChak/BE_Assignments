...
logLikelihood_bivariate <- function(input_alpha, input_beta){
  ...
  result <- sum(sapply(2:n, function(j) 
    ...
    -1/2 * (input_returns[j]^2 / (s_squared * (1-input_alpha) 
                                  + input_alpha * input_returns[j-1]^2
                                  + input_beta * variance[j-1]))))
...
informationTable <- function(input_alpha, input_beta){
...
accepted_df <- informationTable(alpha_posterior, beta_posterior)
accepted_df
...
candidate_df <- informationTable(alpha_candidate_posterior,
